...
# calculate cover different invasive groups per subplot to add to data
forinvcov <- comp.wy %>% filter(species!="BG"& # all invasive sp summed together
                                  species!="Litter"&
                                  native == "I") %>% 
  group_by(year,block,trt,subplot) %>% 
  summarize(invcov = sum(cover, na.rm=T)) #summarize total live veg per subplot
comp.wy.wide <- merge(comp.wy.wide,forinvcov, by=c("year","block","trt","subplot"), all.x = T)

forinvgcov <- comp.wy %>% filter(species!="BG"& # invasive grasses together
                                       species!="Litter"&
                                       native == "I"&
                                  graminoid=="1") %>% #only native live veg
  group_by(year,block,trt,subplot) %>% 
  summarize(invgrass = sum(cover, na.rm=T)) #summarize total live veg per subplot
comp.wy.wide <- merge(comp.wy.wide,forinvgcov, by=c("year","block","trt","subplot"), all.x = T)

# create log response ratio as response metric (not using here though)
...
# create log response ratio as response metric (cannot because subplots were not paired)
...
hist(comp.wy.wide$invcov)
hist(log(comp.wy.wide$invcov)) # not much better logged 
hist(comp.wy.wide$invgrass)
hist(log(comp.wy.wide$invgrass)) # better logged 
...
### visualize
# WY
ggplot(comp.wy.wide, aes(y=log(invgrass), x=nativecov, col=drought))+ #adding drought trt?
  geom_point()+
...
ggplot(comp.wy.wide, aes(y=log(BRTE), x=trt, fill=drought))+ #adding drought trt?
  ...
  facet_wrap(~year)
ggplot(comp.wy.wide, aes(y=log(invcov), x=trt, fill=drought))+ #adding drought trt?
  geom_boxplot()+
  scale_fill_manual(values=c("blue","red"))+
  facet_wrap(~year)
ggplot(comp.wy.wide, aes(y=log(invgrass), x=trt, fill=drought))+ #adding drought trt?
  geom_boxplot()+
  scale_fill_manual(values=c("blue","red"))+
  facet_wrap(~year)
...
m1.wy <- lmer(log.brte ~ trt * nativecov + (1 | block), data = testwy) #not working? random effects?
summary(m1.wy)
m2.wy <- lmer(log.brte ~ trt * nativecov * drought + (1 | block), data = testwy) #not working? random effects?
summary(m2.wy)
anova(m2.wy)
